...
// J'insère tout mon code dans une instruction (try catch) pour regrouper toutes les instructions à exécuter
// et définir une réponse à afficher si l'une des instruction génère une exception
try {
  //-----------------------------------------------------------------------------------------------//
  ...
  // Je récupère le tableau categories et le tableau works sur l'API
  jsonWorks = await getWorks(); //const jsonWorks = await (await fetch(`${urlBase}/works`)).json();
  ...
  const closeModal = function (e) {
    ...
    // J'efface la photo déjà présente dans la modale quand je closeModal
    removePhoto();
  ...
  // Je crée les <options> de la balise <select> en parcourant liste des catégories de l'API
  for (let i = 0; i < jsonCategories.length; i++) {
